"""Named Bearer-token authentication for the AFC MCP server.

Tokens are stored in a JSON file with the following shape::

    {
      "vscode-dev": {
        "token": "afc_xxxxxxxxxxxxxxxxxxxxxxxxxxx",
        "description": "VSCode development client",
        "created": "2026-06-20T10:30:00Z"
      }
    }

The *name* (the JSON key) is the identity of the client that presents the
token. This module is intentionally limited to the Python standard library so
the companion CLI (``afc_token_manager.py``) can run on the host without
installing the MCP dependencies.
"""
...
DEFAULT_TOKENS_FILE = os.environ.get("AFC_TOKENS_FILE", "/app/secrets/.tokens")
